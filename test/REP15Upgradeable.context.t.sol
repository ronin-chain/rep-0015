...
import { REP15UpgradeableTest, ControllerMock, console } from "./REP15Upgradeable.t.sol";
...
contract REP15UpgradeableContextTest is REP15UpgradeableTest {
  ...
  function testFuzz_createContext(uint64 detachingDuration_) public {
    detachingDuration_ = detachingDuration_ % (MAX_DETACHING_DURATION + 1);

    vm.expectEmit(false, true, true, true, address(target));
    emit IREP15.ContextUpdated(bytes32(0), controllerEOA, detachingDuration_);

    target.createContext(controllerEOA, detachingDuration_, "newContext");
  }
